...
namespace Gs2.Installer
{
    ...
    public class WelcomeWindow : EditorWindow
    {
        ...
        [MenuItem ("Game Server Services/インストーラー")]
        ...
        async void OnGUI() {
            ...
            switch (_status)
            {
                ...
                case Status.Installing:
                {
                    ...
                    {
                        var request = Client.Add("io.gs2.unity.weave.editor-extension");
                        while (request.Status == StatusCode.InProgress)
                        {
                            await Task.Delay(1000);
                        }
                    ...
                    }
                    {
                        ...
                        if (request.Result.Count(item => item.name == "io.gs2.unity.sdk") > 0 && 
                            request.Result.Count(item => item.name == "io.gs2.unity.weave.editor-extension") > 0)
                        {
                ...
                case Status.InstallComplete:
                    ...
                    GUILayout.Label("続けて、GS2にログインしましょう。");
                    GUILayout.Label("");
                    if (GUILayout.Button("ログイン"))
                    {
                        PlayerPrefs.SetString("io.gs2.tutorial", true.ToString());
                        PlayerPrefs.Save();

                        var signinWindowType = Type.GetType("Gs2.Editor.Project.SigninWindow, Gs2.Editor");
                        GetWindow(signinWindowType, true, "Sign-in to GS2");
                        Close();
                    }
                    break;
            }
